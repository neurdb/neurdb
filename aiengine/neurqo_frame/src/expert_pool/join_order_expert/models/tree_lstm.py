...
class TreeLSTM(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.layer_norm_input = nn.LayerNorm(
            5 * hidden_size, elementwise_affine=elementwise_affine
        )
        self.layer_norm_left = nn.LayerNorm(
            5 * hidden_size, elementwise_affine=elementwise_affine
        )
        self.layer_norm_right = nn.LayerNorm(
            5 * hidden_size, elementwise_affine=elementwise_affine
        )
        self.layer_norm_c = nn.LayerNorm(
            hidden_size, elementwise_affine=elementwise_affine
        )
    ...
    def forward(self, h_left, c_left, h_right, c_right, feature):
        ...
        c = a.tanh() * i.sigmoid() + f1.sigmoid() * c_left + f2.sigmoid() * c_right
        c = self.layer_norm_c(c)
        h = o.sigmoid() * c.tanh()
        return h, c

    def zero_h_c(self, input_dim=1):
        return torch.zeros(input_dim, self.hidden_size, device=DEVICE), torch.zeros(
            input_dim, self.hidden_size, device=DEVICE
        )
