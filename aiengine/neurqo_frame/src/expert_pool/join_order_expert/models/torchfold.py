...
class Fold(object):
    ...
    class ComputedResult(object):
        ...
        def try_get_batched(self, nodes):
            ...
            if nodes[0].split_idx == -1 and not isinstance(self.result, tuple):
                return self.result
            elif nodes[0].split_idx >= 0 and not isinstance(
                self.result[nodes[0].split_idx], tuple
            ):
        ...
        def get(self, index, split_idx=-1):
            if split_idx == -1:
                ...
                if not isinstance(self.result[split_idx], tuple):
                    self.result[split_idx] = torch.chunk(
                        self.result[split_idx], self.batch_size
                    )
    ...
    def add(self, op, *args):
        ...
        if not all(
            [
                isinstance(arg, (Fold.Node, int, torch._C._TensorBase, Variable))
                for arg in args
            ]
        ):
            raise ValueError(
        ...
        if args not in self.cached_nodes[op]:
            step = max(
                [0] + [arg.step + 1 for arg in args if isinstance(arg, Fold.Node)]
            )
            ...
            if all(isinstance(arg_item, Fold.Node) for arg_item in arg):
                ...
                if arg[0].batch:
                    ...
                    for arg_item in arg[1:]:
                        if arg_item != arg[0]:
                            raise ValueError(
                                "Can not use more then one of nobatch argument, got: %s."
                                % str(arg_item)
                            )
    ...
    def apply(self, nn, nodes):
        ...
        for step in sorted(self.steps.keys()):
            ...
            for op in self.steps[step]:
                func = getattr(nn, op)
                try:
                    # print(" 155 Error while executing node %s[%d] with args: %s" % (
                    # op, step, self.steps[step][op][0]))
                    batched_args = self._batch_args(zip(*self.steps[step][op]), values)
                except Exception:
                    print(
                        "Error while executing node %s[%d] with args: %s"
                        % (op, step, self.steps[step][op][0])
                    )
                    raise
                if batched_args:
...
class Unfold(object):
    ...
    def apply(self, nn, nodes):
        if nn != self.nn:
            raise ValueError(
                "Expected that nn argument passed to constructor and passed to apply would match."
            )
        result = []
        for n in nodes:
