...
class QueryFeaturizer:
    ...
    def __init__(self, workload_info):
        ...
        print(
            "QueryFeaturizer, {} rel_ids".format(len(self.workload_info.rel_ids)),
    ...
    def __call__(self, node):
        ...
        for rel_id in joined:
            idx = np.where(self.workload_info.rel_ids == rel_id)[0][0]
            vec[idx] = self.workload_info.table_num_rows[self.table_id_to_name(rel_id)]
        ...
        all_aliases = [
            self.table_id_to_alias(table) for table in self.workload_info.rel_ids
        ]
        ...
        triu = adj_matrix[np.triu_indices(len(all_aliases), k=1)].astype(np.float32)
