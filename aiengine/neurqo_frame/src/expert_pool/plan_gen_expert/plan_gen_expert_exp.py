...
class Experience:
    ...
    def load(
        ...
        for path in paths:
            ...
            assert initial_size == self.initial_size, (path, initial_size, self.initial_size)
            assert len(nodes) >= initial_size and len(nodes) % initial_size == 0, (len(nodes), path)
            ...
            if keep_last_fraction < 1:
                ...
                print(' orig len {} keeping the last fraction {} ({} iters)'.format(
            ...
            num_unique_plans, unique_plans_table = self._count_unique_plans(self.initial_size, nodes_executed)
            total_num_unique_plans_prev = total_num_unique_plans
            total_num_unique_plans = self._merge_unique_plans_into(unique_plans_table, total_unique_plans_table)
            print(
                ' num_unique_plans from loaded buffer {}; actually new unique plans contributed (after merging) {}'.format(
                    num_unique_plans, total_num_unique_plans - total_num_unique_plans_prev))
    ...
    @staticmethod
    def _count_unique_plans(num_templates: int, nodes: List[workload.Node]) -> Tuple[int, Dict[str, set]]:
        ...
        for i in range(num_templates):
            ...
            for j, node in enumerate(nodes[i::num_templates]):
                assert node.info['query_name'] == query_name, (node.info['query_name'], query_name)
    ...
    @staticmethod
    def _merge_unique_plans_into(from_table: Dict[str, set], into_table: Dict[str, set]) -> int:
    ...
    def prepare(self, rewrite_generic: bool = False) -> None:
        ...
        print('{} rel_ids: {}'.format(len(self.workload_info.rel_ids), self.workload_info.rel_ids))
        ...
        if self.use_tree_conv:
            assert issubclass(self.plan_featurizer_cls, plan_graph_encoder.PhysicalTreeNodeFeaturizer)
            self.plan_featurizer = self.plan_featurizer_cls(self.workload_info)
            self.pos_featurizer = None
        else:
            self.plan_featurizer = self.plan_featurizer_cls(self.workload_info)
            self.pos_featurizer = plan_graph_encoder.ParentPositionFeaturizer(self.workload_info)

        if self.query_featurizer is None:
            self.query_featurizer = self.query_featurizer_cls(self.workload_info)

        if isinstance(self.plan_featurizer, plan_graph_encoder.PreOrderSequenceFeaturizer):
    ...
    def _featurize_dedup(
        ...
        if use_last_n_iters > 0 and use_new_data_only:
            print('Both use_last_n_iters > 0 and use_new_data_only are set: letting the latter take precedence.')
        if on_policy:
            assert use_last_n_iters == -1, 'Cannot have both on_policy and use_last_n_iters={} set.'.format(
                use_last_n_iters)
            use_last_n_iters = 1
        ...
        for i in range(self.initial_size):
            subplan_to_best, num_subtrees, subtrees = self.compute_best_latencies(
                i, skip_first_n, with_physical_hints=not rewrite_generic,
        ...
        return (all_query_vecs, all_feat_vecs, all_pos_vecs, all_costs, num_new_datapoints)
    ...
    def _featurize_hindsight_relabeling(
        ...
        def _top_down_collect(
            ...
            if (node.actual_time_ms is not None or
                    (skip_first_n == 0 and len(self.nodes) == self.initial_size and is_top_level)):
                goal = node
                goal.info = copy.deepcopy(info_to_attach)
                goal_cost = node.cost if node.actual_time_ms is None else node.actual_time_ms
                hindsight_goal_costs.append((goal, goal_cost))
            for goal, goal_cost in hindsight_goal_costs:
                accum.append(SubPlanTrainingPoint(subplan=node, goal=goal, cost=goal_cost))
            for c in node.children:
                _top_down_collect(c, hindsight_goal_costs, accum, info_to_attach, is_top_level=False)

        for i in range(self.initial_size):
            i_start = self.get_first_index_for_template(i, skip_first_n, use_last_n_iters)
            accum = []
            info_to_attach = {k: self.nodes[i].info[k] for k in ['all_filters_est_rows']}
            ...
            if verbose:
                print('{} subplan_to_best, {} entries'.format(self.nodes[i].info['query_name'], len(ret)))
                pprint.pprint(list(ret.values()))

        all_feat_vecs, all_pos_vecs = plan_graph_encoder.make_and_featurize_trees(self.plan_featurizer, all_subtrees)
        print('num_total_subtrees={} num_unique_subtrees={}'.format(num_total_subtrees, len(all_query_vecs)))
    ...
    def featurize(
        ...
        return self._featurize_dedup(
            ...
            skip_training_on_timeouts=skip_training_on_timeouts,
        )[:-1]  # Exclude num_new_datapoints from return
    ...
    def drop_agent_experience(self) -> None:
        ...
        print('Dropped agent experience (prev len {}, new len {})'.format(old_len, len(self.nodes)))
