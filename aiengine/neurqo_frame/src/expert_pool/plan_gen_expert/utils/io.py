...
def set_global_seed(seed=2024):
    ...
    torch.cuda.manual_seed_all(seed)  # Sets seed for all CUDA devices (if multiple GPUs)
...
def save_per_train_plot_data(embeddings, latencies, predictions, query_ids, output_file):
    """
...
def load_per_train_plot_data(input_file):
    ...
    data = np.load(input_file, allow_pickle=True)  # allow_pickle=True for object arrays (query_ids)
