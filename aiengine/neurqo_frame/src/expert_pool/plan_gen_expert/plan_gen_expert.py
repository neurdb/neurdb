...
from common import config_imdb, workload
...
class PerQueryTimeoutController:
    ...
    def OnIterStart(self):
        ...
        if self.curr_iter_max_ms is not None and self.iter_executed:
            # Update timeout.
            if self.iter_timeout_ms is None:
                self.iter_timeout_ms = self.curr_iter_max_ms * self.timeout_slack
            ...
            if (
                self.relax_timeout_factor is not None
                and self.num_consecutive_timeout_iters
...
class BalsaModel(nn.Module):
    ...
    def __init__(
        ...
        reduce_lr_within_val_iter: bool = False,
    ) -> None:
        super().__init__()
    ...
    def compute_loss(
        self, batch: Any, training: bool = True
        ...
        l2_loss = None
        if self.l2_lambda > 0:
            l2_loss = torch.tensor(0.0, device=device, requires_grad=True)
...
class GraphOptimizerExpert:
    ...
    @staticmethod
    def workload_builder(
        db_cli: PostgresConnector, input_sql_dir: str, init_baseline_exp: str
    ):
        search_space_join_ops = ["Hash Join", "Merge Join", "Nested Loop"]
        search_space_scan_ops = [
            "Index Scan",
        ...
        if os.path.isfile(init_baseline_exp):
            ...
            for sql_file_name, sql_str in query_w_name:
                query_node = plan_node.PGToNodeHelper.sql_to_plan_node_w_parsed_sql(
                    db_cli, sql_str, sql_file_name
                )
            ...
            workload_ins.table_num_rows = config_imdb.GetAllTableNumRows(
                workload_ins.rel_names
            )
    ...
    def get_train_sim_model(self):
        ...
        # Instantiate.
        sim = plan_gen_expert_sim.SimModelBuilder(
            params=sim_p, db_cli=self.db_cli, workload_ins=self.workload
        )
    ...
    def run_baseline_optimzier(self):
        ...
        for node in self.workload.nodes:
            ...
            print(
                f'q{node.info["query_name"]},{real_cost:.1f} (baseline), '
    ...
    def _MakeModel(self, dataset, train_from_scratch=False):
        ...
        if not hasattr(self, "model") or p.skip_sim_init_iter_1p:
            ...
            model = treeconv.TreeConvolution(
                feature_size=query_feat_size,
        ...
        model = BalsaModel(
            ...
            learning_rate=(
                self.lr_schedule.Get()
                if self.adaptive_lr_schedule is None
                else self.adaptive_lr_schedule.Get()
            ),
        ...
        if p.agent_checkpoint is not None and self.curr_value_iter == 0:
            ckpt = torch.load(
                p.agent_checkpoint, map_location=lambda storage, loc: storage
            )
    ...
    def Run(self):
        ...
        while self.curr_value_iter < p.val_iters:
            ...
            print(
                f"[Run]. Train one iteration done, with time={end_iteration - begin_iteration}"
            )
            if (
                p.early_stop_on_skip_fraction is not None
                and self.curr_iter_skipped_queries
    ...
    def RunOneIter(self):
        ...
        to_execute, execution_results = self.plan_and_execute(
    ...
    def save_running_result(self, planner, model, iter_total_latency):
        ...
        self.SaveAgent(
            model, iter_total_latency, curr_value_iter=self.curr_value_iter, parameter=p
        )
    ...
    def train_all_epoch(self, train_from_scratch=False):
        ...
        train_ds, train_loader, _, val_loader = self._make_dataloader(
    ...
    def _SampleInternalNode(self, node):
        ...
        def _Sample(subnode, remaining_internal):
            ...
            if np.random.rand() < 1.0 / remaining_internal:
    ...
    def SelectPlan(
        self, found_plans, predicted_latency, found_plan, planner, query_node
    ):
        ...
        if p.epsilon_greedy:
            assert p.epsilon_greedy_random_transform + p.epsilon_greedy_random_plan <= 1
        if p.epsilon_greedy > 0:
            ...
            with torch.no_grad():
                v_values = torch.tensor(
                    [-v for v, _ in found_plans], dtype=torch.float32
                )
            ...
            p.explore_visit_counts
            or p.explore_visit_counts_sort
            or p.explore_visit_counts_latency_sort
        ):
            ...
            if visit_sum > 0:
                ...
                if p.explore_visit_counts:
                    ...
                    if p.explore_visit_counts_latency_sort and all(
                        [x > 0 for x in visit_counts]
                    ):
                        found_plans_sorted, visit_counts_sorted = zip(
                            *sorted(
                                zip(found_plans, visit_counts),
    ...
    def plan_and_execute(self, model, planner: BMOptimizer, is_test=False):
        ...
        for i, node in enumerate(nodes):
            ...
            print(
                "{}q{}, predicted time: {:.1f}".format(
                    "[Test set] " if is_test else "",
            ...
            print(
                f'q{node.info["query_name"]},(predicted {predicted_latency:.1f}),{hint_str}'
            )
            ...
            # --- Direct execution ---
            sql_with_hint = plan_gen_expert_sim._fuse_hints(
                node.info["sql_str"], hint_str
            )
            try:
            ...
            min_p_latency, min_pos = min(
                (pl, pos) for pos, (pl, _) in enumerate(found_plans)
            )
        ...
        query_log_file_name = (
            f"logs/{self.time_temp}__{experiment_cls}__plan_and_execute.txt"
        )
    ...
    def FeedbackExecution(self, to_execute, execution_results):
        ...
        for node, result_tup, to_execute_tup in zip(
            self.train_nodes, execution_results, to_execute
        ):
            try:
                ...
                self.query_execution_cache.Put(
                ...
                self.best_plans.Put(
                    key=node.info["query_name"], value=actual, latency=real_cost
                )
                ...
                self.LogScalars(
                    [
                        (
                            "latency/q{}".format(node.info["query_name"]),
                            real_cost / 1e3,
                            self.curr_value_iter,
                        ),
        ...
        self.LogScalars(
            [
                # Prediction errors.
                (
                    "latency/mean_l1_agent_secs",
                    np.mean(np.abs(agent_plans_diffs)),
                    self.curr_value_iter,
                ),
                (
                    "latency/mean_pred-tgt_agent_secs",
                    -np.mean(agent_plans_diffs),
                    self.curr_value_iter,
                ),
                (
                    "latency/mean_l1_expert_secs",
                    np.mean(np.abs(expert_plans_diffs)),
                    self.curr_value_iter,
                ),
                (
                    "latency/mean_pred-tgt_expert_secs",
                    -np.mean(expert_plans_diffs),
                    self.curr_value_iter,
                ),
                # Timeout metrics.
                (
                ...
                ("num_total_timeouts", self.num_total_timeouts, self.curr_value_iter),
    ...
    def _make_dataloader(self, log=True):
        ...
        train_ds, train_loader, val_ds, val_loader, num_train, num_validation = (
    ...
    def initialize_model(
        ...
        if tau:
            if not soft_assign_use_ema:
                print(
                    "Assigning real model := {}*SIM + {}*previous real model".format(
                        1 - tau, tau
                    )
                )
                ...
                assert isinstance(
                    ema_source_tm1, collections.OrderedDict
                ), ema_source_tm1
                ...
                with torch.no_grad():
                    ...
                    for key, param in model_weights.items():
                        ema_source_t[key] = (
                            tau * ema_source_tm1[key] + (1.0 - tau) * param
                        )
        ...
        if p.param_noise:
            for layer in model.out_mlp:
                if isinstance(layer, nn.Linear):
                    print(
                        "Adding N(0, {}) to out_mlp's {}.".format(p.param_noise, layer)
                    )
    ...
    def LogTimings(self):
        ...
        if (
            p.test_query_glob is not None
            and num_iters_done >= p.test_after_n_iters
            and num_iters_done % p.test_every_n_iters == 0
        ):
        ...
        data_to_log = []
        for stage, timing, cumulative_timing in zip(
            stages, timings, cumulative_timings
        ):
            data_to_log.extend(
                [
                    # Time, this iter.
                    ...
                    (
                        "timing_pct/{}".format(stage),
                        timing / iter_total_s,
                        self.curr_value_iter,
                    ),
                    # Total time since beginning.
                    (
                        "timing_cumulative/{}".format(stage),
                        cumulative_timing,
                        self.curr_value_iter,
                    ),
    ...
    def _LogDatasetStats(self, train_labels, num_new_datapoints):
        ...
        if num_new_datapoints is not None:
            data.append(
                ("train/num-new-datapoints", num_new_datapoints, self.curr_value_iter)
            )
        self.LogScalars(data)

    def LogExpertExperience(
        self,
        expert_train_nodes: List[workload.Node],
        expert_test_nodes: List[workload.Node],
    ):
        p = self.params
        total_s = 0
        data_to_log = []
        ...
        print(
            "latency_expert/workload (seconds): {:.2f} ({} queries)".format(
    ...
    def EvaluateTestSet(self, model, planner, tag="latency_test"):
        ...
        num_iters_done = self.curr_value_iter + 1
        if (
            p.test_query_glob is None
            or num_iters_done < p.test_after_n_iters
            or num_iters_done % p.test_every_n_iters != 0
        ):
            return
        if p.test_using_retrained_model:
            print("[Test set] training a new model just for test set reporting.")
    ...
    def SaveAgent(
        self, model, iter_total_latency, curr_value_iter=None, parameter=None
    ):
        ...
        ckpt_path = os.path.join(
            base_save_dir, f"{parameter.model_prefix}_checkpoint.pt"
        )
        ...
        if curr_value_iter is not None:
            base_folder_path = os.path.join(
                base_save_dir, f"{parameter.model_prefix}_checkpoints", self.time_temp
            )
            if not os.path.exists(base_folder_path):
                os.makedirs(base_folder_path)
            intermediate_path = os.path.join(
                base_folder_path, f"checkpoint__iter{curr_value_iter}.pt"
            )
            try:
                torch.save(state_dict, intermediate_path)
            except Exception as e:
                print(
                    f"Error saving intermediate checkpoint to {intermediate_path}: {e}"
                )
                raise
        ...
        io.save_text(
            ...
            os.path.join(
                base_save_dir, f"{parameter.model_prefix}_checkpoint-metadata.txt"
            ),
        )
    ...
    def LogTestExperience(self, to_execute_test, execution_results, tag="latency_test"):
        ...
        agent_plans_diffs = []
        expert_plans_diffs = []
        for node, to_execute, result_tup in zip(
            self.test_nodes, to_execute_test, execution_results
        ):
            ...
            data.append(
                (
                    "{}/q{}".format(tag, node.info["query_name"]),
                    real_cost / 1e3,
                    self.curr_value_iter,
                )
            )
        ...
        if has_timeouts:
            ...
            # written out.
            print("[Test set {}] timeout events detected during eval".format(tag))
            return
        ...
        data.extend(
            [
                ...
                (
                    tag + "/mean_l1_agent_secs",
                    np.mean(np.abs(agent_plans_diffs)),
                    self.curr_value_iter,
                ),
                (
                    tag + "/mean_pred-tgt_agent_secs",
                    -np.mean(agent_plans_diffs),
                    self.curr_value_iter,
                ),
                (
                    tag + "/mean_l1_expert_secs",
                    np.mean(np.abs(expert_plans_diffs)),
                    self.curr_value_iter,
                ),
                (
                    tag + "/mean_pred-tgt_expert_secs",
                    -np.mean(expert_plans_diffs),
                    self.curr_value_iter,
                ),
                ("num_query_execs", self.num_query_execs, self.curr_value_iter),
        ...
        if tag == "latency_test":
            self.overall_best_test_latency = min(
                self.overall_best_test_latency, iter_total_latency / 1e3
            )
    ...
    def generate_query_search_history(
        ...
        messages.append(
            "{} Execution time: {:.1f} (predicted {:.1f}) curr_timeout_ms={}".format(
                query_name, real_cost, predicted_latency, curr_timeout_ms
            )
        )
        ...
        messages.append(
            "  {:.1f} (predicted {:.1f})  {}".format(
                query_node.cost,
                query_node.info["curr_predicted_latency"],
                expert_hint_str,
            )
        )
        if found_plans:
            ...
            for p_cost, found in zip(predicted_costs, found_plans):
                ...
                extras = [
                    ...
                    (
                        "[expert plan]"
                        if found_hint_str_physical == expert_hint_str_physical
                        else ""
                    ),
    ...
    def check_hint_respected(
        ...
        if hint_str is not None:
            ...
            if plan_json is not None:
                ...
                executed_node, _ = plan_node.PGToNodeHelper.sql_to_plan_node(
                    cursor=self.db_cli, sql=sql_str, comment=hint_str
                )
...
if __name__ == "__main__":
    ...
    from data_collection.collect_data_unified import BufferManager, read_sql_files
    ...
    class BalsaParams:
        ...
        def Params(cls):
            ...
            p.define("engine", "postgres", "The execution engine.  Options: postgres.")
            p.define(
                "engine_dialect_query_dir",
                None,
                "Directory of the .sql queries in target engine's dialect.",
            )

            p.define(
                "run_baseline", False, "If true, just load the queries and run them."
            )
            ...
            p.define(
                "cost_model", "postgrescost", "A choice of postgrescost, mincardcost."
            )

            p.define("bushy", True, "Plans bushy query execution plans.")

            p.define(
                "search_space_join_ops",
            ...
            p.define(
                "lr_piecewise",
                None,
                "If supplied, use Piecewise.  Example:" "[(0, 1e-3), (200, 1e-4)].",
            )
            ...
            p.define(
                "reduce_lr_within_val_iter", False, "Reduce LR within each val iter?"
            )
            ...
            p.define(
                "increment_iter_despite_timeouts",
                False,
            ...
            p.define("adamw", None, "If not None, the weight_decay param for AdamW.")
            p.define(
                "label_transforms", ["log1p", "standardize"], "Transforms for labels."
            )
            # p.define('label_transform_running_stats', False,
            #          'Use running mean and std to standardize labels?' \
            #          '  May affect on-policy.')
            p.define(
                "update_label_stats_every_iter",
                True,
            ...
            p.define(
                "test_using_retrained_model",
                False,
                "Whether to retrain a model from scratch just for testing.",
            )
            p.define(
                "track_model_moving_averages", False, "Track EMA/SWA of the agent?"
            )
            ...
            p.define(
                "init_baseline_experience",
                "initial_policy_data.pkl",
            ...
            p.define(
                "prev_replay_keep_last_fraction",
                1,
            ...
            p.define("model_save_path", "./", "Where to save the model ")

            p.define("model_prefix", "", "Model prefix, for naming the model")
            return p
    ...
    parser.add_argument(
        "--db_path",
        default="buffer_imdb_ori_db",
        help="Path to unified SQLite database",
    )
    parser.add_argument(
        "--input_sql_dir",
        type=str,
