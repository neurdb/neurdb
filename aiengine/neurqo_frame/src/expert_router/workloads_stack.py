...
def load_and_process_data(cfg: BaseConfig, with_leon=False) -> pd.DataFrame:
    ...
    df.loc[df[df["planning_time"] >= cfg.EXECUTION_TIME_OUT].index, "planning_time"] = (
        0.0
    )
    ...
    aggregated_dfs = []
    split_df = (
        df.groupby(["experiment", "query_ident", "split"])
        .count()
        .reset_index()
        .sort_values(["experiment", "query_ident"])[
            ["experiment", "query_ident", "split"]
        ]
    )

    for (method, run_type, experiment), df_group in df.groupby(
        ["method", "run_type", "experiment"], dropna=False
    ):
        if run_type != "regular":
            continue

        df_group.loc[df_group.index, "timed_out"] = df_group.loc[
            df_group.index, "timed_out"
        ].astype(int)

        x = df_group.groupby(
            ["method", "run_type", "experiment", "query_ident", "split"], dropna=False
        )
        aggregated = x.agg(
            {
        ...
        aggregated.loc[aggregated["timed_out"] > 0, "execution_time"] = (
            cfg.EXECUTION_TIME_OUT
        )
        # Add correction to total_time
        aggregated["total_time"] = (
            aggregated["inference_time"]
            + aggregated["planning_time"]
        ...
        if method != "PostgreSQL":
            ...
            for new_experiment in df["experiment"].dropna().unique():
                ...
                tmp["split"] = split_df[split_df["experiment"] == new_experiment][
                    "split"
                ].tolist()
...
def split_train_test_queries(
    test_queries_dict: Dict[str, List[str]],
    cfg: BaseConfig,
    test_size: int = 20,
    seed: int = None,
    ...
    test_queries = set(
        random.sample(
            test_queries_candidates, min(test_size, len(test_queries_candidates))
        )
    )
    ...
    print(
        f"Total queries: {total_queries}, Train: {len(train_queries)}, Test: {len(test_queries)}"
    )
    return train_queries, test_queries


def compute_avg_performance(
    df: pd.DataFrame, queries: List[str], split_type: str = "train"
) -> pd.DataFrame:
    ...
    if df_queries.empty:
        logging.warning(
            f"No data found for {split_type} split with queries: {queries[:5]}..."
        )
    return (
        df_queries.groupby(["query_ident", "method"])["total_time"].mean().reset_index()
    )
...
def assign_labels(avg_performance: pd.DataFrame) -> pd.DataFrame:
    ...
    for query in avg_performance["query_ident"].unique():
        ...
        labels = {
            ...
            "top_2_methods": (
                list(top_methods[:2]) if len(top_methods) >= 2 else list(top_methods)
            ),
...
def prepare_workload_test_datasets(
    ...
    for workload, query_idents in test_queries_dict.items():
        ...
        for query in df_workload["query_ident"].unique():
            ...
            sorted_data = query_data.sort_values(
                "total_time"
            )  # Sort by total_time ascending
            methods = sorted_data["method"].values

            execution_time_dict = dict(
                zip(query_data["method"], query_data["total_time"])
            )
            labels = {
                "query_ident": query,
                "top_1_method": methods[0],
                "top_2_methods": (
                    list(methods[:2]) if len(methods) >= 2 else list(methods)
                ),  # Top 2 methods
                "execution_time_ms": json.dumps(execution_time_dict),
            }
            labeled_data.append(labels)
        ...
        # Add raw SQL strings
        df_labeled["query_sql"] = df_labeled["query_ident"].apply(
            lambda q: cfg.load_sql_query(q)
        )
        ...
        print(
            f"Saved dataset for '{workload}' to '{output_file}' with {len(df_labeled)} rows"
        )
...
def prepare_workload_train_test_datasets(
    ...
    for workload, _ in test_queries_dict.items():
        ...
        df_train = df_workload[df_workload["split"] == "train"][
            ["query_ident", "method", "total_time"]
        ].copy()
        df_test = df_workload[df_workload["split"] == "test"][
            ["query_ident", "method", "total_time"]
        ].copy()
        ...
        # Add raw SQL strings
        df_train_labeled["query_sql"] = df_train_labeled["query_ident"].apply(
            lambda q: cfg.load_sql_query(q)
        )
        df_test_labeled["query_sql"] = df_test_labeled["query_ident"].apply(
            lambda q: cfg.load_sql_query(q)
        )

        # Save to disk
        train_output_file = os.path.join(
            save_dir, f"workload_{workload}_train_data.csv"
        )
        ...
        print(
            f"Saved train dataset for '{workload}' to '{train_output_file}' with {len(df_train_labeled)} rows"
        )
        print(
            f"Saved test dataset for '{workload}' to '{test_output_file}' with {len(df_test_labeled)} rows"
        )
...
def prepare_router_dataset(
    ...
    for dataset in [train_labeled, test_labeled]:
        dataset["query_sql"] = dataset["query_ident"].apply(
            lambda q: cfg.load_sql_query(q)
        )
    ...
    train_labeled.to_csv(
        os.path.join(folder_name, f"train_data_{timestamp}.csv"), index=False
    )
    test_labeled.to_csv(
        os.path.join(folder_name, f"test_data_{timestamp}.csv"), index=False
    )
    print(
        f"Router datasets saved: {folder_name}/train_data_{timestamp}.csv, {folder_name}/test_data_{timestamp}.csv"
    )
...
def load_merged_datasets(folder_name: str) -> Dict[str, pd.DataFrame]:
    ...
    for split in ["train", "test"]:
        datasets[split]["execution_time_ms"] = datasets[split][
            "execution_time_ms"
        ].apply(json.loads)
...
def load_workload_test_datasets(base_dir) -> Dict[str, pd.DataFrame]:
    ...
    workload_files = sorted(
        glob.glob(os.path.join(base_dir, "workload_*_test_data.csv"))
    )  # Fixed pattern
    for file_path in workload_files:
        ...
        datasets[workload_name]["execution_time_ms"] = datasets[workload_name][
            "execution_time_ms"
        ].apply(json.loads)
...
def load_workload_train_test_datasets(
    folder_name: str,
) -> Dict[str, Dict[str, pd.DataFrame]]:
    datasets = {}
    ...
    for train_file in train_files:
        ...
        for split in ["train", "test"]:
            datasets[workload][split]["execution_time_ms"] = datasets[workload][split][
                "execution_time_ms"
            ].apply(json.loads)
...
def validate(df_agg):
    ...
    df_sum = (
        df_agg.groupby(["base_experiment", "experiment", "method", "split"])
        .agg(
            {
                "total_time": "sum",
                "execution_time": "sum",
                ...
                "prepare_time": "sum",
                "total_time.std": "sum",
                "execution_time.std": "sum",
                "planning_time.std": "sum",
                "inference_time.std": "sum",
    ...
    df_sum["execution_time.err"] = (
        1.96 * df_sum["execution_time.std"] / np.sqrt(df_sum["n"])
    )
    df_sum["planning_time.err"] = (
        1.96 * df_sum["planning_time.std"] / np.sqrt(df_sum["n"])
    )
    df_sum["inference_time.err"] = (
        1.96 * df_sum["inference_time.std"] / np.sqrt(df_sum["n"])
    )
    df_sum["total_time.err"] = 1.96 * df_sum["total_time.std"] / np.sqrt(df_sum["n"])
    df_sum["prepare_time.err"] = (
        1.96 * df_sum["prepare_time.std"] / np.sqrt(df_sum["n"])
    )
    return df_sum
