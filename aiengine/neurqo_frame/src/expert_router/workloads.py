...
def load_and_process_data(cfg: BaseConfig, with_leon=False) -> pd.DataFrame:
    ...
    df_regular = df[
        ~df["experiment"].isin(imdb_experiments) & (df["run_type"] == "regular")
    ]
    ...
    aggregated_dfs = []
    split_df = (
        df_regular.groupby(["experiment", "query_ident", "split"])
        .size()
        .reset_index()
    ...
    for (method, experiment), df_group in df_regular.groupby(
        ["method", "experiment"], dropna=False
    ):
        df_group["timed_out"] = df_group["timed_out"].astype(int)
        grouped = df_group.groupby(
            ["method", "experiment", "query_ident", "split"], dropna=False
        )
        agg_dict = {
        ...
        aggregated.columns = [
            f"{col[0]}.std" if col[1] == "std" else col[0] for col in aggregated.columns
        ]
