...
class OnlineRouter:
    BufferEntry = namedtuple(
        "BufferEntry",
        ["X_batch", "chosen_optimizer", "observed_latency", "timestamp", "weight"],
    )
    ...
    def __init__(
        ...
        self.buffer = deque(
            maxlen=buffer_size
        )  # (features, optimizer, latency, timestamp, weight)
        ...
        self.avg_regret = 0.0
        plogger.info(
            f"OnlineRouter initialized with DCTS, buffer_size={buffer_size}, lr={lr}"
        )
    ...
    def select_optimizer(self, X_batch: dict, query_id=None):
        ...
        with torch.no_grad():
            ...
            offline_pred_idx = torch.argmin(
                reg_times_off.squeeze(0).masked_fill(~offline_valid_mask, float("inf"))
                if offline_valid_mask.sum() > 0
                else reg_times_off.squeeze(0)
            )
            offline_mean = reg_times_off.squeeze(0)  # Use as prior mean

            self.model.train()  # Enable dropout for TS
            dropout_samples = max(
                3, self.dropout_samples - (self.query_count // 50)
            )  # Min 3 samples, reduce over time
            sampled_latencies = torch.zeros(
                dropout_samples, self.output_dim, device=device
            )
            ...
            variance = sampled_latencies.var(
                dim=0, unbiased=True
            )  # Use unbiased variance
            variance = torch.nan_to_num(
                variance, nan=1e-6, posinf=1e6
            )  # Handle nan/inf
            ...
            if (
                max_variance > threshold and self.query_count < 50
            ):  # Explore early, then trust learning
                ...
                plogger.info(
                    f"High uncertainty: {max_variance:.4f}, Explored, Choice: {chosen_optimizer}"
                )
            else:
                chosen_optimizer = torch.argmin(
                    mean_latencies
                ).item()  # Exploit learned mean
                plogger.info(
                    f"Low uncertainty: {max_variance:.4f}, Exploited, Choice: {chosen_optimizer}"
                )

            self.model.eval()
            return chosen_optimizer

    def add_to_buffer_update_model(
        self,
        X_batch: dict,
        chosen_optimizer: int,
        ...
        weight = 1.0
        X_batch_np = {
            key: value.squeeze(0).cpu().numpy() for key, value in X_batch.items()
        }
        entry = self.BufferEntry(
            X_batch_np, chosen_optimizer, observed_latency, timestamp, weight
        )
        ...
        if (
            self.avg_regret > 0.2 * best_pred_latency
            and len(self.regret_window) == self.regret_window.maxlen
        ):
            plogger.info(f"Shift detected: avg regret={self.avg_regret:.4f}")
            self.flush_old_buffer()

        if (
            self.query_count % self.update_frequency == 0
        ...
        for x in X_batch:
            ...
            if pad_filters > 0:
                pad_tensor = torch.full(
                    (pad_filters, filters.shape[1]), 0.0
                )  # Pad with 0
    ...
    def update_model(self):
        ...
        chosen_optimizers = torch.tensor(
            [item.chosen_optimizer for item in batch], device=self.cfg.DEVICE
        )
        observed_latencies = torch.tensor(
            [item.observed_latency for item in batch], device=self.cfg.DEVICE
        )
        ...
        alpha = 0.5 + 0.3 * min(
            self.avg_regret / (self.avg_regret + 1), 1.0
        )  # Cap at 0.8
        ...
        self.model.eval()
        plogger.info(
            f"Updated with loss={loss.item():.4f}, alpha={alpha:.4f}, regret={self.avg_regret:.4f}"
        )
