...
class PGToNodeHelper:
    ...
    @staticmethod
    def _fuse_hints(
        sql: str, comment: str, explain_str: str = "explain(verbose, format json)"
    ):
    ...
    @staticmethod
    def sql_to_plan_node(
        cursor: PostgresConnector,
        sql: str,
        comment: str = None,
        ...
        new_sql = PGToNodeHelper._fuse_hints(
            sql=sql, comment=comment, explain_str="explain(verbose, format json)"
        )
    ...
    @staticmethod
    def sql_to_plan_node_w_parsed_sql(
        db_cli: PostgresConnector, sql_str, sql_file_name
    ):
        ...
        graph, join_conds = node.get_parse_update_sql(
    ...
    @staticmethod
    def estimate_nodes_selectivility(
        db_cli: PostgresConnector, nodes: List[workload.Node]
    ):
        """EstimateFilterRows
            ...
            for table_id, pred in node.info["all_filters"].items():
                ...
                if key not in cache:
                    ...
                    try:
                        ...
                        print(
                            f"[estimate_nodes_selectivility]: cannot explian sql: {e}, use full table rows estimates"
                        )
                    ...
                    cache[key] = num_rows
        print(
            "{} unique filters, with example <(table_id, filter): number rows> = {}".format(
...
class Stats:
    ...
    def Print(self):
        ...
        def DoPrint(cnts):
            df = pd.DataFrame(cnts, index=["count"]).T.sort_values(
