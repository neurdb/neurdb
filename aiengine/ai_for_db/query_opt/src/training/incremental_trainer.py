...
class IncrementalCheckpointTrainer:
    ...
    def _target_indices(self, transition: dict[str, Any]) -> dict[str, int]:
        ...
        decision_states = _canonical_decision_states(transition.get("decision_states"))
    ...
    def _logits_for_target(
        ...
        if self.model_method != "standardmdp_rl":
            raise RuntimeError("the NQO incremental trainer requires standardmdp_rl")
        if phase == "dec":
...
def main() -> int:
    ...
    ap.add_argument("--db-log", required=True, help="DB-side nqo.trajectory_log JSONL")
