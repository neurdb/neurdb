 ...
 * @member {torch::jit::script::Module*} module - the wrapped torch::jit::script::Module
 */
typedef struct {
 ...
 * @param {int*} dims - the dimensions of the tensor, e.g., [3, 4] for a 3x4 matrix
 * @param {int} n_dim - the number of dimensions, e.g., 2 for a 3x4 matrix
 * @return {TensorWrapper*} - the created tensor
 */
TensorWrapper *
tw_create_tensor(float *data, const int *dims, int n_dim);
 ...
 * @param {TensorWrapper*} tw_tensor - the tw_tensor to get the number of dimensions from
 ...
 * @param {cstring} model_serialized_data - the pickle serialized data of the model
 * @param {size_t} size - the size of the serialized data
 * @return {ModelWrapper*} - the loaded model
 */
ModelWrapper *
tw_load_model_by_serialized_data(const char *model_serialized_data, size_t size);

/**
 * @description: Feed tw_forward the input tensor through the tw_model and get the output tensor
