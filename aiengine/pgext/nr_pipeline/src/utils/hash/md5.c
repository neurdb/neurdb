...
static int compute_md5(const char *str, unsigned char *md5_hash, unsigned int *md5_length) {
...
char *nr_md5_str(const char *string) {
    ...
    if (compute_md5(string, md5_hash, &md5_length)) {
        char *result = (char *) malloc(md5_length * 2 + 1); // each byte is represented by 2 hex characters
