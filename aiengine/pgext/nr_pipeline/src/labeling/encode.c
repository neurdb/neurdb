...
int encode_text(char *text, char *table_name, char *column_name) {
  int encoded_value = 0;
  char *mapping_table =
      psprintf("%s_%s_encode_mapping", table_name, column_name);
  ...
  char *select_query = psprintf(
      "SELECT encoded_value FROM %s WHERE text = '%s'", mapping_table, text);

  SPI_execute(select_query, true, 0);
  if (SPI_processed == 0) {
    // the mapping does not exists
    char *insert_query = psprintf(
        "INSERT INTO %s (text, encoded_value) VALUES ('%s', DEFAULT) RETURNING "
        "encoded_value",
        mapping_table, text);
...
int encode_column(char *table_name, char *column_name) {
  char *query = psprintf("SELECT DISTINCT %s FROM %s", column_name, table_name);
...
void create_mapping_table_if_not_exists(char *table_name, char *column_name) {
  char *mapping_table =
      psprintf("%s_%s_encode_mapping", table_name, column_name);
  char *query = psprintf(
      "CREATE TABLE IF NOT EXISTS %s (text TEXT PRIMARY KEY, encoded_value "
      "SERIAL UNIQUE)",
      mapping_table);
