...
Datum pgm_predict_float4(PG_FUNCTION_ARGS) {
    ...
    if (SRF_IS_FIRSTCALL()) {
        ...
        if (model == NULL) {
            ...
            elog(WARNING,
                 "pgm_predict_float4: model %d is not found, no prediction is "
                 "made",
                 model_id);
        ...
        if (output == NULL) {
            ...
            elog(WARNING,
                 "pgm_predict_float4: error occurred during model forward "
                 "inference, "
        ...
        const int column_num = (int)prediction_result_data
                                   ->dims[prediction_result_data->n_dims - 1];

        TupleDesc tupdesc = CreateTemplateTupleDesc(column_num);
        for (int i = 0; i < column_num; i++) {
            char column_name[32];
            sprintf(column_name, "output_%d", i + 1);
            TupleDescInitEntry(tupdesc, (AttrNumber)(i + 1), column_name,
                               FLOAT4OID, -1, 0);
        }
...
Datum pgm_predict_table(PG_FUNCTION_ARGS) {
    ...
    appendStringInfo(&query, "SELECT %s FROM %s", column_names_array,
                     table_name);
    ...
    for (int start = 0; start < num_rows; start += batch_size) {
        ...
        for (int i = start; i < end; ++i) {
            ...
            for (int j = 0; j < num_columns; ++j) {
                bool isnull;
                data[(i - start) * num_columns + j] = DatumGetFloat4(
                    SPI_getbinval(tuple, tupdesc, j + 1, &isnull));
            }
        }
        ...
        if (output == NULL) {
            ...
            pfree(data);
            ereport(ERROR,
                    (errmsg("pgm_predict_table: error occurred during model "
                            "forward inference")));
            PG_RETURN_NULL();
        }
        batch++;
