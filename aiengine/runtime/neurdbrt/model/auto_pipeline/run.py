...
if __name__ == "__main__":
    ...
    from auto_pipeline import config, util
    from auto_pipeline.config import default_config as conf
    from auto_pipeline.ctxpipe.dataset import Dataset
...
class DatasetBuilder:
    ...
    def build(self):
        self._parse_task_info(os.path.join(self._info.dataset_prefix, ""))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--dataset-path", type=str, required=True, help="dataset path"
    )

    parser.add_argument("--model-step", type=int, default=32000)
