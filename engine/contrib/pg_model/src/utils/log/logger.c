...
void
logger_init(Logger *logger, int capacity) {
...
void
logger_start(Logger *logger, char *message) {
    if (logger->size >= logger->capacity) {
        // double the capacity
        logger->capacity *= 2;
        logger->records = (LogRecord *) realloc(logger->records, logger->capacity * sizeof(LogRecord));
...
void
logger_end(Logger *logger) {
    LogRecord *record = &logger->records[logger->size];
    clock_gettime(CLOCK_REALTIME, &record->timer.end);

    // record the duration in milliseconds
    const double elapsed_time = (double)(record->timer.end.tv_sec - record->timer.start.tv_sec) * 1e3 +
...
void
logger_export(Logger *logger, const char *filename) {
