...
{
	List	   *p_target = NIL; /* A list of targets (columns) for the
								 * prediction */
	ListCell   *o_target;
	...
	/* Extract the column names from targetList */
	foreach(o_target, stmt->targetList)
	{
		...
		p_target = lappend(p_target, colname);
	}

	/* Combine column names into a single string */
	foreach(o_target, p_target)
	{
		char	   *targetName = (char *) lfirst(o_target);

		if (targetName == NULL)
		{
			elog(ERROR, "Null target name in target list");
			return InvalidObjectAddress;
		}
		appendStringInfo(&columns, "%s,", targetName);
	}
