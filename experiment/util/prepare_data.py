...
def prepare(number_of_rows: int = 150) -> None:
    ...
    cursor.execute('SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = \'iris\')')
    ...
    if current_row_num > number_of_rows:
        raise Exception(f'Number of rows in iris table is {current_row_num}, greater than {number_of_rows}')

    cursor.execute('SELECT * FROM iris')
    existing_rows = cursor.fetchall()
    insert_query = "INSERT INTO iris (sepal_l, sepal_w, petal_l, petal_w, class) VALUES %s"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_rows', type=int, default=150, help='Number of rows in iris table')
