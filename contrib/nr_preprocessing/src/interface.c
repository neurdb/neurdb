  ...
  while (true) {
    ...
    for (int i = 0; i < SPI_processed; i++) {
      ...
      for (int col = 0; col < n_features; col++) {
        ...
        switch (type) {
          case INT2OID:appendStringInfo(&row_data, " %d:%hd", col + 1, DatumGetInt16(value));
            break;
          case INT4OID:appendStringInfo(&row_data, " %d:%d", col + 1, DatumGetInt32(value));
            break;
          case INT8OID:appendStringInfo(&row_data, " %d:%ld", col + 1, DatumGetInt64(value));
            break;
          case FLOAT4OID:appendStringInfo(&row_data, " %d:%f", col + 1, DatumGetFloat4(value));
            break;
          case FLOAT8OID:appendStringInfo(&row_data, " %d:%lf", col + 1, DatumGetFloat8(value));
            break;
          case TEXTOID:
          case VARCHAROID:
          case CHAROID:
