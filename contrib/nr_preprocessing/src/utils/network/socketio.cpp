...
struct SocketIOClient {
    ...
    BatchDataQueue *queues[4] = {nullptr};
};
...
void socketio_set_queues(SocketIOClient *client, BatchDataQueue *queues[4]) {
    client->queues[0] = queues[0];
    client->queues[1] = queues[1];
    client->queues[2] = queues[2];
    client->queues[3] = queues[3];
}

BatchDataQueue *socketio_get_queue(const SocketIOClient *client, MLStage stage) {
    return client->queues[stage];
}
...
void socketio_register_callback(SocketIOClient *client, const char *event, MessageCallback callback) {
    client->callbacks[event] = callback;
    client->client.socket()->on(event, [client, event](const sio::event &response) {
        // message returned should be a JSON object
        cJSON *json = cJSON_CreateObject();
        for (const auto &item: response.get_message()->get_map()) {
            cJSON_AddStringToObject(json, item.first.c_str(), item.second->get_string().c_str());
        }

        auto callback = client->callbacks.find(event); // find the callback function
