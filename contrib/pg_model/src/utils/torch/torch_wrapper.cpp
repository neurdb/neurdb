...
TensorWrapper *
tw_create_tensor(float *data, const int *dims, const int n_dim) {
    const std::vector<int64_t> dims_int64(dims, dims + n_dim); // compose the dimensions
    const auto tw_tensor = new TensorWrapper();
    // ArrayRef<int64_t> is a reference to the dimensions of the tensor
    const auto tensor = new torch::Tensor(torch::from_blob(data, at::ArrayRef<int64_t>(dims_int64), at::kFloat));
...
float *
tw_get_tensor_data(const TensorWrapper *tw_tensor) {
    const auto tensor = static_cast<torch::Tensor *>(tw_tensor->tensor); // cast void* to torch::Tensor*
    return tensor->data_ptr<float>();
}

long *
tw_get_tensor_dims(const TensorWrapper *tw_tensor) {
...
long
tw_get_tensor_n_dim(const TensorWrapper *tw_tensor) {
    // dim() returns the number of dimensions, yet sizes() returns the sizes of each dimension
...
ModelWrapper *
tw_load_model_by_path(const char *model_path) {
    const auto tw_model = new ModelWrapper();
    const auto module = new torch::jit::script::Module(torch::jit::load(model_path));
    module->to(*static_cast<torch::Device *>(device)); // move module to device
    tw_model->module = module;
    return tw_model;
}

ModelWrapper *tw_load_model_by_serialized_data(const char *model_serialized_data, const size_t size) {
    const auto tw_model = new ModelWrapper();
    ...
    script_module->to(*static_cast<torch::Device *>(device)); // move module to device
    tw_model->module = script_module;
    return tw_model;
}

TensorWrapper *
tw_forward(const ModelWrapper *tw_model, const TensorWrapper *input) {
    const auto module = static_cast<torch::jit::script::Module *>(tw_model->module);
...
bool
tw_save_model(const char *model_name, const char *save_path, const ModelWrapper *tw_model) {
    // concatenate the save path and tw_model name
    const std::string full_save_path = std::string(save_path) + "/" + std::string(model_name) + ".pt";
    try {
        const auto module = static_cast<torch::jit::script::Module *>(tw_model->module);
        module->save(full_save_path);
        return true;
    } catch (const c10::Error &e) {
        std::cerr << "tw_save_model: error saving tw_model: " << e.what() << std::endl;
        return false;
    }
}

char *
tw_serialize_model(const ModelWrapper *tw_model, size_t *size) {
    const auto module = static_cast<torch::jit::script::Module *>(tw_model->module); // the model to be serialized
...
void
tw_free_model(const ModelWrapper *tw_model) {
    if (tw_model) {
...
void
tw_free_tensor(const TensorWrapper *tw_tensor) {
    if (tw_tensor) {
