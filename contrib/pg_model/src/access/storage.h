 ...
 * @param init_params the initialization parameters of the layer, e.g. "in_features=10, out_features=5"
 ...
 */
LayerStorage *
create_layer_storage(const char *layer_class, const char *init_params, void *state_dict, size_t state_dict_size,
                     const char *name);
 ...
 */
void *
serialize_layer_storage(const LayerStorage *layer_storage, size_t *serialized_length);
 ...
 */
void *
deserialize_layer_storage(const void *data, size_t length);

/**
 * Convert a layer storage to a model wrapper, this can be done since layers in PyTorch are also nn.Modules
 * @param layer_storage the layer storage to be converted
 * @return the model wrapper
 */
ModelWrapper *
to_model_wrapper(const LayerStorage *layer_storage);
 ...
 */
void
free_layer_storage(LayerStorage *layer_storage);
...
/**
 * PickledModelStorage is used to store the pickled model, this is used to store the model in the database
 */
typedef struct {
 ...
 * @param init_params the initialization parameters of the model, e.g. "layers=[torch.nn.Linear(10, 5), torch.nn.ReLU()]"
 * @param layers the layers of the model
 * @param layer_count the number of layers
 * @return the model storage
 */
ModelStorage *
create_model_storage(const char *model_class, const char *init_params, LayerStorage **layers,
                     size_t layer_count);
 ...
 */
ModelStorage *
from_model(const ModelWrapper *model);
 ...
 */
void
append_layer(ModelStorage *model_storage, LayerStorage *layer_storage);
 ...
 */
ModelWrapper *
to_model(const ModelStorage *model_storage);
 ...
 */
PickledModelStorage *
serialize_model_storage(const ModelStorage *model_storage);
 ...
 */
ModelStorage *
deserialize_model_storage(const PickledModelStorage *pickled_model_storage);
 ...
 */
void
free_model_storage(ModelStorage *model_storage);
 ...
 */
void
free_pickled_model_storage(PickledModelStorage *pickled_model_storage);
