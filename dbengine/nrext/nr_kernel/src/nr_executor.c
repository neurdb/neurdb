...
static void ExecCheckXactReadOnly(PlannedStmt *plannedstmt) {
  ...
  foreach (l, plannedstmt->permInfos) {
    ...
    if ((perminfo->requiredPerms & (~ACL_SELECT)) == 0) continue;

    if (isTempNamespace(get_rel_namespace(perminfo->relid))) continue;
...
static void InitPlan(QueryDesc *queryDesc, int eflags) {
   ...
   * However, the actual locking happens during execution when rows are accessed
   * for the first time
   */
  if (plannedstmt->rowMarks) {
    ...
    foreach (l, plannedstmt->rowMarks) {
      ...
      if (relation) CheckValidRowMarkRel(relation, rc->markType);
...
void NeurDB_ExecutorStart(QueryDesc *queryDesc, int eflags) {
  ...
  switch (queryDesc->operation) {
    ...
    case CMD_PREDICT:
      ...
      /*
       * Bypass the trigger which are often associated with INSERT, UPDATE, and
       * DELETE operations
       */
      eflags |= EXEC_FLAG_SKIP_TRIGGERS;
      break;
...
void NeurDB_ExecutorRun(QueryDesc *queryDesc, ScanDirection direction,
  ...
  if (queryDesc->totaltime) InstrStartNode(queryDesc->totaltime);
  ...
  /*
   * run plan,
   * if if (!ScanDirectionIsNoMovement(direction)): query execution requires
   * actual tuple processing
   */
...
void NeurDB_ExecutorEnd(QueryDesc *queryDesc) {
  if (original_executorend_hook) original_executorend_hook(queryDesc);
...
void NeurDB_ExecutorFinish(QueryDesc *queryDesc) {
  if (original_executorfinish_hook) original_executorfinish_hook(queryDesc);
  ...
  if (queryDesc->totaltime) InstrStopNode(queryDesc->totaltime, 0);
