...
func (c *SQLDB) init() {
	...
	c.mustExec(fmt.Sprintf("ALTER SYSTEM SET default_transaction_isolation = '%s'",
		configs.DefaultIsolationLevel))
...
func (c *Shard) ReadTxnPostgres(tableName string, txnID uint32, key uint64) (*RowData, bool) {
	...
	if configs.LockType == configs.OCC && ok {
