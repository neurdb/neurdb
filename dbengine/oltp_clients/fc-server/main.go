...
func init() {
	...
	flag.IntVar(&l, "len", 10, "the transaction length")
	...
	flag.IntVar(&warm, "warmup", 2, "the warmup time")
	flag.IntVar(&runTime, "runtime", 2, "the run time")
...
func main() {
	...
	configs.RunTestInterval = runTime
	configs.WarmUpTime = time.Duration(warm) * time.Second
