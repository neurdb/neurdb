...
def generate_embeddings(args, config_args):
    ...
    builder._model.load_state_dict(
...
def save_clusters(data, labels, args, top_clusters):
    ...
    sorted_clusters = sorted(
        cluster_mapping.items(), key=lambda x: len(x[1]), reverse=True
    )[:top_clusters]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_clusters", type=int, default=5, help="Number of clusters for K-Means"
    )
    parser.add_argument(
        "--top_clusters",
        type=int,
        default=4,
        help="Number of top largest clusters to save",
    )

    parser.add_argument(
        "--config_path",
        type=str,
        default="/Users/kevin/project_c++/neurdb_proj/neurdb-dev/contrib/nr/pysrc/config.ini",
    )
