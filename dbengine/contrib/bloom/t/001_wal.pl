...
SELECT * FROM tst WHERE i = 7 AND t = 'e';
	...
	my $primary_result = $node_primary->safe_psql("neurdb", $queries);
	my $standby_result = $node_standby->safe_psql("neurdb", $queries);
...
$node_primary->safe_psql("neurdb", "CREATE EXTENSION bloom;");
$node_primary->safe_psql("neurdb", "CREATE TABLE tst (i int4, t text);");
$node_primary->safe_psql("neurdb",
...
for my $i (1 .. 10)
{
	$node_primary->safe_psql("neurdb", "DELETE FROM tst WHERE i = $i;");
	test_index_replay("delete $i");
	$node_primary->safe_psql("neurdb", "VACUUM tst;");
