...
public:
    SELIXIndexEngine() {
        elog(LOG, "SELIX IndexEngine: initialized with default SELIX config");
    }
    ...
    void bulkLoad(Oid indexOid, int64_t* keys, uint64_t* values, int count) {
        ...
        elog(LOG, "SELIX: bulkLoad completed, indexOid=%u, count=%zu, data_nodes=%d, model_nodes=%d",
...
IndexEngine* indexengine_open(void) {
    try {
        return reinterpret_cast<IndexEngine*>(new SELIXIndexEngine());
...
void indexengine_close(IndexEngine* engine) {
    if (engine) {
        delete reinterpret_cast<SELIXIndexEngine*>(engine);
    }
}
...
void indexengine_put(IndexEngine* engine, NRIndexKey ikey, NRIndexValue ivalue) {
    ...
    try {
        SELIXIndexEngine* impl = reinterpret_cast<SELIXIndexEngine*>(engine);
