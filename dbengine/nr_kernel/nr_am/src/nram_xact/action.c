...
void init_agent_function_cache(void) {
    bool found = false;

    /* Allocate/attach a single shared CachedAgentFuncData block */
    CachedAgentFuncData *ptr = (CachedAgentFuncData *)ShmemInitStruct(
        NRAM_AGENTFUNC_SHMEM_NAME, sizeof(CachedAgentFuncData), &found);
...
void load_agent_function(const char *filename) {
    FILE *fp;
    char line[256];
    uint32 lineno = 0;

    if (GlobalCachedAgentFunc == NULL)
        ereport(ERROR, (errmsg("GlobalCachedAgentFunc not initialized"),
                        errhint("Call init_agent_function_cache() during "
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (n == 4) {
            if (idx >= MAX_XACT_FEATURE_SPACE)
                ereport(
                    ERROR,
                    (errmsg(
                        "load_agent_function: index %u out of range at line %u",
                        idx, lineno)));

            GlobalCachedAgentFunc->act[idx].detect_all = (detect != 0);
            GlobalCachedAgentFunc->act[idx].priority = prio;
            GlobalCachedAgentFunc->act[idx].timeout =
                (uint32)Min(tout, (unsigned long long)UINT32_MAX);
            continue;
        }
        ...
        if (n == 3) {
            if (cursor >= MAX_XACT_FEATURE_SPACE)
                ereport(ERROR,
                        (errmsg("load_agent_function: too many entries (>%d)",
                                MAX_XACT_FEATURE_SPACE)));

            GlobalCachedAgentFunc->act[cursor].detect_all = (detect != 0);
            GlobalCachedAgentFunc->act[cursor].priority = prio;
            GlobalCachedAgentFunc->act[cursor].timeout =
                (uint32)Min(tout, (unsigned long long)UINT32_MAX);
            cursor++;
            continue;
        }
        ...
        ereport(ERROR,
                (errmsg("load_agent_function: malformed line %u in \"%s\"",
                        lineno, filename),
                 errdetail("Expected: 'idx detect_all priority timeout_ns' or "
                           "'detect_all priority timeout_ns'.")));
    }

    fclose(fp);
}

CCAction get_action(XactFeature feature) {
    if (GlobalCachedAgentFunc == NULL)
        ereport(ERROR, (errmsg("GlobalCachedAgentFunc not initialized"),
                        errhint("Call init_agent_function_cache() during "
                                "shared memory startup.")));
...
void print_xact_feature(StringInfo str, XactFeature feature) {
    if (feature == NULL) {
        appendStringInfoString(str, "<XactFeature NULL>");
        return;
    }

    appendStringInfo(str, "{n_access=%u, cur_op=%s, n_dep=%u, enc_idx=%u}",
                     feature->n_access,
                     (feature->cur_op == READ_OPT)     ? "READ"
                     : (feature->cur_op == UPDATE_OPT) ? "WRITE"
                                                       : "UNKNOWN",
