    ...
    def set_bounds(self):
        ...
        if self.setting["timeout"]:
            timeout_parameters.extend([
                ng.p.Scalar(lower=1, upper=100000, init=100_000) for _ in range(self.max_state)
            ])
            check_length += self.max_state
...
class Policy(object):
    def __init__(self, _access=None, _rank=None, _timeout=None,  _from=None, load_file=None, encoded=None):
        self.score = -1
        self.learner = _from
        ...
        if load_file is not None:
            ...
            self.access = np.array(_access if _access is not None else np.zeros(self.max_state, dtype=int))
            self.rank = np.array(_rank if _rank is not None else np.zeros(self.max_state, dtype=float))
            self.timeout_policy = np.array(_timeout if _timeout is not None else np.full(self.max_state, 100000.0))
    ...
    def decode(self, param_dict):
        ...
        if self.learner.setting["access"]:
            ...
            self.access = (bp.access.copy() if (bp is not None and hasattr(bp, "access"))
        ...
        if self.learner.setting["rank"]:
            ...
            self.rank = (bp.rank.copy() if (bp is not None and hasattr(bp, "rank"))
        ...
        if self.learner.setting["timeout"]:
            ...
            self.timeout_policy = np.array(timeout_values[-self.max_state:], dtype=float)
            ...
            if bp is not None and hasattr(bp, "timeout_policy"):
                ...
                self.extra_policies = np.array([32] + [2 for _ in range(6 * N_TXN_TYPE)], dtype=int)
