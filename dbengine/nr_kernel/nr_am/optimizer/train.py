...
def evaluate():
    ...
    parser.add_argument(
        "--base-log-dir",
        type=str,
        default="./optimizer/bo-all",
        help="model save location",
    )
    parser.add_argument(
        "--base-kid-dir",
        type=str,
        default="./optimizer/bo",
        help="kid policy save location",
    )
    parser.add_argument("--expr-name", type=str, default="bo", help="experiment name")
    ...
    parser.add_argument(
        "--nworkers", type=int, default=8, help="number of database workers"
    )
    parser.add_argument(
        "--pickup-policy",
        type=str,
    ...
    parser.add_argument(
        "--state-space", type=int, default=32, help="state space for policy searching"
    )
    ...
    parser.add_argument(
        "--duration", type=int, default=5, help="benchmark duration seconds"
    )
    parser.add_argument("--warmup", type=int, default=2, help="warmup seconds")
    parser.add_argument(
        "--txn-length", type=int, default=10, help="ops per transaction"
    )
    parser.add_argument("--nkeys", type=int, default=10000, help="number of keys")
    parser.add_argument(
        "--payload-bytes", type=int, default=0, help="payload size per row"
    )
    parser.add_argument(
        "--workload",
        type=str,
        default="A",
        choices=["A", "B", "C", "F", "custom"],
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="silence runner stdout/stderr"
    )
