    ...
    async with conn.transaction(isolation=args.isolation.lower()):
        ...
        for _ in range(args.txn_length):
            ...
            if op == "READ":
                ...
                await conn.execute(
                    f"UPDATE {table} SET balance = {data} WHERE id = {k}"
                )
                stats["writes"] += 1
            else:  # RMW
                ...
                await conn.execute(
                    f"UPDATE {table} SET balance = {val + delta} WHERE id = {k}"
                )
                stats["rmw"] += 1
...
RETRYABLE_SQLSTATES = {
    ...
    "40P01",
    "55P03",
}  # serialization, deadlock, lock timeout (optional)
MAX_RETRIES = 5
...
async def worker(pool, sampler, args, stats, stop_event):
    table = args.table
    mix = WORKLOADS.get(
        args.workload.upper(), (args.read_ratio, 1.0 - args.read_ratio, 0.0)
    )
...
async def main():
    p = argparse.ArgumentParser(
        description="YCSB-like benchmark for neurdb (NRAM table)."
    )
    p.add_argument(
        "--dsn",
        default=os.environ.get("PG_DSN", "postgres://127.0.0.1/neurdb"),
    ...
    p.add_argument(
        "--payload-bytes", type=int, default=0, help="Size of text padding per row"
    )
    ...
    p.add_argument(
        "--workload",
        default="A",
        choices=list(WORKLOADS.keys()) + ["custom"],
    ...
    p.add_argument(
        "--zipf-theta", type=float, default=0.8, help="Zipf skew (0=uniform)"
    )
    p.add_argument(
        "--isolation",
    ...
    p.add_argument(
        "--preload", action="store_false", help="Prepopulate table to n-keys"
    )
    p.add_argument(
        "--policy", default=None, help="Call SELECT nram_load_policy('<name>')"
    )  # No policy for postgres.
    args = p.parse_args()
    ...
    rc = subprocess.call(["make", "setup"], stdout=subprocess.DEVNULL)
    if rc != 0:
    ...
    async def init_conn(conn, iso_sql):
        await conn.execute(
            f"SET SESSION CHARACTERISTICS AS TRANSACTION ISOLATION LEVEL {iso_sql}"
        )

    iso_sql = args.isolation.upper()
    pool = await asyncpg.create_pool(
        dsn=args.dsn,
        min_size=args.threads,
        max_size=args.threads,
    )
    ...
    if args.warmup > 0:
        ...
        tasks = [
            asyncio.create_task(worker(pool, sampler, args, manager, warm_stop))
    ...
    # Timed run
    print(
        f"[run] threads={args.threads} duration={args.duration}s workload={args.workload} iso={args.isolation} policy={args.policy or '(unchanged)'}"
    )
    tasks = [
        asyncio.create_task(worker(pool, sampler, args, manager, stop))
        for _ in range(args.threads)
    ]
    ...
    if lats:
        print(
            "Txn latency ms : p50={:.2f}  p90={:.2f}  p95={:.2f}  p99={:.2f}  max={:.2f}".format(
